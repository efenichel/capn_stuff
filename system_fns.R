...
cropFwater <- function(water, param){
  ...
  contr <- exp(as.numeric(cropSharePar$alpha) + water * as.numeric(cropSharePar$beta))
  h <- sum(contr) 
  contr <- c(contr, 1)
...
DcropFwater <- function(water, param){
  ...
  contr <- exp(as.numeric(cropSharePar$alpha) + water * as.numeric(cropSharePar$beta))
  h <- sum(contr)
  hp <- sum(contr * as.numeric(cropSharePar$beta))
  
  contr <- c(contr, 1)
  bv <- c(as.numeric(cropSharePar$beta), 0)
...
Wwd <- function(water, param){
  ...
  wx <- exp(alpha + water * beta + sum(z * Gamma0)) / sum(AP1) #projects based on regession model
